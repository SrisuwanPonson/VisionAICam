...
using VisionAICam_Linux.Services;

namespace VisionAICam_Linux.Core
{
    /// <summary>
    ...
    public sealed class MasterController : IAsyncDisposable, IDisposable
    {
        ...
        private T EnsureOnUi<T>(ref T? field, Func<T> factory) where T : class
        {
            ...
            try
            {
                // Invoke synchronously on UI thread and wait for completion.
            ...
            {
                try { _logger.LogError($"EnsureOnUi invoke failed: {ex}"); } catch { }
            }
        ...
        public void AddDetectionResults(IEnumerable<VisionAICam.Pages.DetectionResult> results)
        {
            ...
            if (Interlocked.Exchange(ref _flushPending, 1) == 0)
            {
                ...
                // Post the flush operation to the UI thread
                dispatcher.Post(FlushQueuedResults, DispatcherPriority.Normal);
            }
        }
